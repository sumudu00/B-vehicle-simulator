...
public class VehicleControllerGear : MonoBehaviour
	{
	...
	//public FourWheelGearInput FoorwheelScript;
	//public float inputVariablr;
	...
	public static VehicleControllerGear current = null;
	...
	void Update ()
		{
			//inputVariablr = FoorwheelScript.throttleInputb;

			//Debug.Log("The Thrttle input from Fourwheelgearscript" + FoorwheelScript.throttleInputb);

			//Debug.Log("The Thrttle input from vehiclecontrollergearscript" + throttleInput);
			//Debug.Log("Steer Angle "+ m_steerAngle);
			//EngineSound ();
	...
	void EngineSound () {
		float SpeedCurrent = (Mathf.Abs (speed)) * 3.6f;
	...
	void ComputeSteerAngle ()
		{
		inputSteerAngle = maxSteerAngle * steerInput;
		//Debug.Log("inputSteerAngle: " + inputSteerAngle);
    ...
    //        if (throttleInput > 0)
    //        {
    //            byte[] b = System.BitConverter.GetBytes((int)throttleInput);
    //            serial.Write(b, 0, 1);
    //        }
    //    }
	...
	void UpdateTransform (WheelData wd)
		...
		if (wd.wheel.wheelTransform != null || wd.wheel.caliperTransform != null)
			...
			if (wheelPositionMode == PositionMode.Fast)
                ...
                Vector3 wheelPosition = wd.transform.position;
