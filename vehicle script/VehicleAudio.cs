...
namespace EVP
{
    ...
    [RequireComponent(typeof(VehicleControllerGear))]
    public class VehicleAudio : MonoBehaviour
    {
        ...
        [NonSerialized]
        public float skidRatioChangeRate = 40.0f;
        [NonSerialized]
        public float offroadSpeedChangeRate = 20.0f;
        [NonSerialized]
        public float offroadCutoutSpeed = 0.02f;
        [NonSerialized]
        public float dragCutoutSpeed = 0.01f;
        [NonSerialized]
        public float turboRatioChangeRate = 8.0f;
        [NonSerialized]
        public float wheelsRpmChangeRateLimit = 400.0f;
        ...
        public float m_engineThrottleRpm = 0.0f;
        ...
        private SerialCommunicationReadScript SerialScript;
        private FourWheelGearInput GearInputScript;
        private PanalMetersScript MeterSerialScript;

        public AudioSource IgnitionSound;
        public AudioSource SignalSound;
        public AudioSource HornSound;
        public AudioSource SirenSounddd;
        public AudioSource NoiseSounddd;
        public Button NoiseSoundButt;
        bool SirenActive = false;
        bool NoiseActive = false;
        int SirenCount = 0;
        public float updatedEngineRpm;
        public float RPM;
        public float engineVolume = 0;
        ...
        void OnEnable()
        {
            ...
            m_vehicle = GetComponent<VehicleControllerGear>();
            GearInputScript = GetComponent<FourWheelGearInput>();
            SerialScript = GetComponent<SerialCommunicationReadScript>();
            //MeterSerialScript = GetComponent<PanalMetersScript>();
        ...
        void Update()
        {
            ...
            VehicleSpeed = m_vehicle.speed * 3.6f;

            //Setting the Gear from GearInputScriptssssssssssssss
            ...
            {
                //if (GearInputScript.index == 7)
                //{
                //    m_gear = 1;
                //}
                //else
                //{
                //    m_gear = GearInputScript.index;
                //}

                if (GearInputScript.index == 0)
                {
                    m_gear = 1;
                }
                else
            ...
            if (SirenCount < 100 && SirenActive)
            {
                SirenCount++;
            }
            else
            {
                SirenSounddd.enabled = false;
                SirenCount = 0;
                SirenActive = false;
            }

            if (GearInputScript.SystemIsON || /*GearInputScript.SerialSystemIsON == 1*/  SerialScript.Ignition == 1)
            {

                if (SerialScript.Serial)
                {
                    //if (GearInputScript.SerialIgnitionIsOn)
                    if (SerialScript.Ignition == 1)
                    {
                        IgnitionSound.enabled = true;
                    }
                    else
                        IgnitionSound.enabled = false;
                }
                else
                {
                    if (GearInputScript.IgnitionIsON)
                    {
                        IgnitionSound.enabled = true;
                    }
                    else
                ...
                if (GearInputScript.SignalON)
                {
                    SignalSound.enabled = true;
                }
                else
                {
                    SignalSound.enabled = false;
                }

                if ((GearInputScript.SystemIsON && GearInputScript.IgnitionIsON) || (/*GearInputScript.SerialSystemIsON == 1 && GearInputScript.SerialIgnitionIsOn*/ SerialScript.Ignition ==1))
                {

                    //HornSound.enabled = (SerialScript.Horn == 1) ? true : false;

                    /*if (GearInputScript.index < 4)
                    {
                        engine.gearUpRpm = 2500;
                    }
                    else
                    {
                        engine.gearUpRpm = 3000;
                    }*/

                    if (FourWheelGearInput.autoGear)
                    {
                        if (GearInputScript.ODButton)
                        {
                            engine.gears = 3;
                        }
                        else
                    ...
                    {
                        //MeterSerialScript.RPMMeter = 600;
                    }
                    else if (engineVolume > 0.8f)
                    {
                        //MeterSerialScript.RPMMeter = 4000;
                    }
                    else
                    {
                        //MeterSerialScript.RPMMeter = (int)(((engineVolume - 0.2f) * 3400 / 0.6f) + 600);
                    }
                ...
                {
                    OnDisable();
                    //MeterSerialScript.RPMMeter = 0;

                    if (GearInputScript.SerialSystemIsON == 1)
                    {
                        //HornSound.enabled = (SerialScript.Horn == 1) ? true : false;
                    }
            ...
            else
            {
                ...
                //NoiseActive = false;
                //NoiseSounddd.enabled = false;
                //NoiseSoundButt.GetComponentInChildren<Text>().text = "Generate Squeaky Belt Noise";
            }
        ...
        {
            SirenSounddd.enabled = true;
            SirenActive = true;
        }

        public void BeltNoiseSound()
        {
            if (NoiseActive == true)
            {
                NoiseActive = false;
            ...
            {
                NoiseActive = true;
                NoiseSounddd.enabled = true;
                NoiseSoundButt.GetComponentInChildren<Text>().text = "Remove Squeaky Belt Noise";
            }
        ...
        void DoEngineAudioAuto()
        {
            ...
            if (m_vehicle.throttleInput == 0)
            {
                transmissionRpm = 0;
            }
            else
            {
                transmissionRpm = m_wheelsRpm * engine.wheelsToEngineRatio[m_gear];
            }
            ...
            else if (transmissionRpm >= 0)
            {
                ...
                if (transmissionRpm < firstGear || engine.gears <= 1)
                {
                    m_gear = 1;
        ...
        void DoEngineAudio()
        {
            ...
            else if (transmissionRpm >= 0)
            {
                ...
                float gearWidth = engine.maxRpm - engine.idleRpm;
                ...
                //else
                //{
                updatedEngineRpm = Mathf.Repeat(transmissionRpm, gearWidth) + engine.idleRpm;
                //Mathf.Repeat(transmissionRpm - firstGear, gearWidth) + engine.gearDownRpm;
                // }
                //}
            }
            else
            ...
            if (m_vehicle.throttleInput == 0)
            {
                m_engineThrottleRpm = 0;
            }
            else
            {
                m_engineThrottleRpm = Mathf.Lerp(m_engineThrottleRpm, Mathf.Abs(m_vehicle.throttleInput) * engine.throttleRpmBoost, m_engineRpmDamp * Time.deltaTime);
            }

            RPM = m_engineRpm + m_engineThrottleRpm;
            ...
            if (engine.audioSource != null)
            {
                ...
                engineVolume = Mathf.Lerp(engine.idleVolume, engine.maxVolume, engineRatio)
                    + Mathf.Abs(m_vehicle.throttleInput) * engine.throttleVolumeBoost;
                ProcessVolume(engine.audioSource, engineVolume, engine.volumeChangeRateUp, engine.volumeChangeRateDown);
            }
        }
        ...
        void DoEngineExtraAudio()
        {
            ...
            float transmissionRatio = Mathf.Abs(m_wheelsRpm * engine.wheelsToEngineRatio[m_gear]) / (engine.maxRpm * engine.gears * engineExtras.transmissionMaxRatio);
        ...
        void VehicleHorn()
        {
            if (Input.GetKey(KeyCode.S) || SerialScript.Horn == 1)
            {
        ...
        void VehicleBrakeSound()
        {
            if (/*Input.GetKey(KeyCode.S) ||*/ SerialScript.Brake < 370 && VehicleSpeed > 20)
            {
        ...
        //void VehicleHornSerial()
        //{
        //    if (SerialScript.Horn == 1)
        //    {
        //        HornSound.enabled = true;
        //    }
        //    else
