...
public class VehicleControllerAuto : MonoBehaviour
	{
	...
	public float ClutchBrakeTorque = 800.0f;
	...
	public static VehicleControllerAuto current = null;
	...
	void OnValidate ()
		...
		ClutchBrakeTorque = Mathf.Max(ClutchBrakeTorque, 0.0f);
	...
	void FixedUpdate ()
		...
		clutchInput = Mathf.Clamp (clutchInput, -1.0f, +1.0f);
	...
	void EngineSound () {
		float SpeedCurrent = Mathf.Abs (speed);
		for (int i = 0; i < GearRatio.Length; i++){
			if (GearRatio[i] > SpeedCurrent){
				Gear = i;
				break;
			}
		}
		...
		float EnginePitch = ((SpeedCurrent - GearMinValue)/(GearMaxValue - GearMinValue)) + 1;
	...
	void ComputeTireForces (WheelData wd)
		...
		float wheelClutchInput = wd.wheel.drive? clutchInput : 0.0f;
		...
		if (wd.isBraking)
			...
			{
			/* if (InputScript.ClutchHold) {
				float balancedDriveForce = GetRampBalancedValue(maxDriveForce, driveBalance, wd.positionRatio);
			demandedForce = ComputeDriveForce((wd.finalInput * balancedDriveForce - absClutchInput*ClutchBrakeTorque), balancedDriveForce, wd.grounded);
			}
			
			else if (InputScript.ClutchRelease) {
				float balancedDriveForce = GetRampBalancedValue(maxDriveForce , driveBalance, wd.positionRatio);
			demandedForce = ComputeDriveForce((wd.finalInput * balancedDriveForce + absClutchInput*maxDriveForce/4), balancedDriveForce, wd.grounded);
			}
	...
	void OnCollisionEnter (Collision collision)
		...
		if (collision.transform != transform && collision.contacts.Length != 0) {
			for (int i = 0; i < collision.contacts.Length; i++){
				//Instantiate(Spark, collision.contacts[i].point, Quaternion.identity);
				GameObject clone = Instantiate(CollisionSound, collision.contacts[i].point, Quaternion.identity);
